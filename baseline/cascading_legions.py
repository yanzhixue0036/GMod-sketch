...
from estimators import bloom_filters
...
class CascadingLegions(base.SketchBase):
  ...
  def get_sketch_factory(cls, l, m):
    def factory(random_seed):
      return cls(l, m, random_seed) # create an instance of the class
    return factory
  ...
  def get_bucket(self, f):
    ...
    while f % 2 == 0 and (not f == 0):
      if time.time()-start_time > 1000: print(f)
      legion += 1
      f //= 2
  ...
  def add_fingerprint(self, f):
    ...
    # self.mask[b] = self.mask.get(b, set()) | {f}    # get the unoin
    self.sketch[b] = self.sketch.get(b, 0) + 1

  def add_id(self, item):
    """Add id to the estimator."""
    f = farmhash.hash32withseed(str(item), self.seed) # return an integer
