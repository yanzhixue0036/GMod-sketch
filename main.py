...
def get_args():
    ...
    parser.add_argument('--intersection', default=100000, type=int, help='set intersection cardinality') 
    ...
    parser.add_argument('--noise', action='store_true', help='whether add noise')
    ...
    # SFM param
    parser.add_argument('--sfm_Msize', default=2048, type=int, help='m of SFM sketch')
    ...
    # GMOD/GMOD_MEC param
    parser.add_argument('--GMod_Msize', default=2048, type=int, help='m of GMOD/GMOD_MEC sketch')
    parser.add_argument('--GMod_Wsize', default=32, type=int, help='w of GMOD/GMOD_MEC sketch')
    parser.add_argument('--GMod_gsize', default=2, type=int, help='g of GMOD/GMOD_MEC sketch')
    parser.add_argument('--random_response',default=True, type=bool, help='whether use random response')
    
    # Cascading_Legions param
    parser.add_argument('--cl_Msize', default=2048, type=int, help='m of Cascading_Legions')
    parser.add_argument('--cl_l', default=32, type=int, help='l of Cascading_Legions')
    
    # Liquid_Legions param
    parser.add_argument('--ll_Msize', default=2048*32, type=int, help='m of Liquid_Legions')
    parser.add_argument('--ll_a', default=10, type=int, help='a of Liquid_Legions')
    
    # HalfXor param
    parser.add_argument('--Hx_lamb', default=1/1024, type=float, help='lamb of HalfXor')
    parser.add_argument('--Hx_m', default=1024, type=int, help = 'm of HalXor')
...
if __name__ == "__main__":
    ...
    for r in range(exp_rounds):
        ...
        if not args.noise: args.difference = 0
        ...
        if args.method == 'GMOD' and args.noise == True:
            g_plus = int(min(args.GMod_gsize, math.log(math.exp(args.epsilon)+1)))
            gmod = GMod(dict_dataset, args.GMod_Msize, args.GMod_Wsize, g_plus,args.output,seed,args.epsilon, args.random_response,delete_dataset)
            gmod.build_sketch()
        ...
        if args.method == 'GMOD' and args.noise == False:
            ...
            gmod.build_sketch()
            result = gmod.estimate_cardinality() # 
            lst_all_results.append(result[0])
        
        if args.method == 'GMOD_MEC' and args.noise == True:
            g_plus = int(math.log(min(pow(2, args.GMod_gsize), math.exp(args.epsilon)+1)))
            gmod = GMod(dict_dataset,args.GMod_Msize, args.GMod_Wsize, g_plus,args.output,seed,args.epsilon, args.random_response,delete_dataset)
            gmod.build_sketch()
        ...
        if args.method == 'GMOD_MEC' and args.noise == False:
            gmod = GMod(dict_dataset, args.GMod_Msize, args.GMod_Wsize, args.GMod_gsize,args.output,seed,args.epsilon, False,delete_dataset)
            gmod.build_sketch()
            result = gmod.estimate_cardinality_MEC()
            lst_all_results.append(result[0])
        ...
        print('{}th exp_rounds finished!'.format(r))
    ...
    RRMSE = compute_rrmse(lst_all_results, (int)(args.intersection + args.difference)*(1-args.delete_ratio)) #
    print("The value of RRMSE: {}%".format(RRMSE * 100))